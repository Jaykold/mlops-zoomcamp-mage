...
def test_training_test(
    ...
    *args
    ...
    ), f'Entire dataset should have 105870 examples, but has {X_train.shape[0]}'

    assert(
        X_train.shape[1] == 5094
    ), f'Entire dataset should have 5094 features, but has {X_train.shape[1]}'

    assert(
        len(y_train) == X_train.shape[0]
    ), f'Entire dataset should have {X_train.shape[0]} examples, but has {len(y_train.index)}'

@test
def test_validation_test(
    X: csr_matrix,
    X_train: csr_matrix,
    X_val: csr_matrix,
    y: Series,
    ...
    *args
    ...
    assert(
        X_val.shape[0] == 51492
    ), f'Entire dataset should have 51492 examples, but has {X_val.shape[0]}'

    assert(
        X_val.shape[1] == 5094
    ), f'Entire dataset should have 5094 features, but has {X_val.shape[1]}'

    assert(
        len(y_val) == X_val.shape[0]
    ), f'Entire dataset should have {X_val.shape[0]} examples, but has {len(y_val.index)}'